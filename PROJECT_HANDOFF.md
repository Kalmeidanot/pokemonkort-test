...
Shows mock marketplace sales tables from `marketplace.js`. The visible page heading is "Markedsplassen".
Supports filtering by sale type and by active/ended status.
Sales tables are seller/listing groups, not individual products. Each sales table can contain multiple product cards.
Each sales table preview shows seller name, title, status, product count, sale type summary, optional "Fra ... kr", and a small product-image carousel with "1 av N" / "+N flere".
The "Se salgsbod" button navigates in the same tab to `listing.html?id={listing.id}`.
...
Loads `auth.js?v=3`, then `marketplace.js?v=5`, then `listing.js?v=2`.
Reads `id` from the URL, e.g. `listing.html?id=listing-1`, and finds the listing in `MOCK_LISTINGS`.
Shows seller name, sales table title, status, product count, sale type summary, and every product/card in the listing.
...
Product tabs filter the product grid: Alle, Fastpris, Claim, Auksjon, Auksjon uten minstepris, Lynauksjon, Avsluttet.
...
- ~~Første som vises på forsiden skal være markedsplassen med salgsbord-preview.~~ — **done** med frontend-only mock-data i `marketplace.js`.
- Salgstyper:
  - Fastprisannonse
  - Claim-salg
  - Auksjon
  ...
  - Lynauksjon
- Bruker kan filtrere salgsbord etter salgstype.
- Bruker kan se aktive eller avsluttede salgsbord.
- Salgsbord-preview viser selgernavn, tittel, status, antall produkter, salgsformer, eventuell "Fra ... kr" og en enkel produkt-karusell med "1 av N" / "+N flere".
- "Se salgsbod" navigerer til `listing.html?id=...`.
